...
// import { Appointment } from "@/src/stores/appointmentStore";
...
const AppointmentCall = ({
  ...
  // ✅ Appointment stable ref mein rakho — re-render pe intializeCall nahi badlega
  const appointmentRef = useRef(appointment);
  const onCallEndRef = useRef(onCallEnd);
  const joinConsultationRef = useRef(joinConsultation);

  // ✅ Refs update karte raho latest values ke saath
  useEffect(() => {
    appointmentRef.current = appointment;
    onCallEndRef.current = onCallEnd;
    joinConsultationRef.current = joinConsultation;
  });
  ...
  const intializeCall = useCallback(async (container: HTMLDivElement) => {
    ...
    try {
      ...
      // ✅ Ref se latest appointment lo
      const apt = appointmentRef.current;

      try {
        await joinConsultationRef.current(apt._id);
      } catch (error) {
      ...
      const isVideoCall = apt.consultationType === "Video Consultation";
      ...
      zp.joinRoom({
        ...
        onJoinRoom: () => {
          if (isComponentMountedRef.current) {
            console.log(`Joined ${apt.consultationType} : ${apt.zegoRoomId}`);
          }
        },
        onLeaveRoom: () => {
          if (isComponentMountedRef.current && zpRef.current) {
            try {
              zpRef.current.mutePublishStreamAudio(true);
              zpRef.current.mutePublishStreamVideo(true);
            } catch (error) {
        ...
        onUserJoin: (users: any[]) => {
          if (isComponentMountedRef.current) console.log("Users Joined", users);
        },
        onUserLeave: (users: any[]) => {
          if (isComponentMountedRef.current) console.log("Users left", users);
        },
        showLeavingView: true,
        onReturnToHomeScreenClicked: () => {
          if (zpRef.current) {
  ...
  }, [currentUser.id, currentUser.name]); // ✅ Sirf ye 2 stable values — appointment ref se lo
  ...
  // ✅ Safe access — optional chaining se crash nahi hoga
  const otherPersonName = currentUser.role === "doctor"
